...
The QA pipeline proceeds as follows:

\begin{enumerate}
    \item \textbf{Document preparation:} load the SRS, normalize if needed, and split into overlapping chunks.
    ...
    \item \textbf{Generation:} condition a chat model on those chunks to produce an answer.
    \item \textbf{Scoring:} compare the answer to the reference with embedding cosine similarity (Table~\ref{tab:notation}).
\end{enumerate}

This ordering localizes failures to retrieval or generation.
